...
| **DEFER-1.1.3-1** | TASK-1.1.3 (JSON-RPC) | eth_* RPC methods (Ethereum compatibility) | Runtime pallets integrated but node-side infrastructure incomplete | ✅ **COMPLETED** (November 24, 2025) | ✅ Complete | **IMPLEMENTATION COMPLETE:** (1) ✅ Runtime: Frontier (Moonbeam fork) pallets at indices 10-12 in construct_runtime with full EVM configuration (frontier.rs 212 lines) - pallet_evm, pallet_ethereum, pallet_base_fee fully configured with ChainId=200, BlockGasLimit=15M, EIP-1559 base fees, precompiles, (2) ✅ Node: Core Frontier RPC dependencies wired (fc-rpc, fc-db, fc-mapping-sync, fp-rpc) - spawn_frontier_tasks function complete, Frontier backend initialized, mapping sync worker spawned, (3) ✅ RPC: Actual EthApi, NetApi, Web3Api, EthFilter, EthPubSub, TxPool APIs fully implemented in frontier.rs, (4) ✅ VERSION FIXES: Moonbeam fork branch, parity-scale-codec 3.7.5, sc-network-types 0.21.0 for kad module compatibility. **Status:** Code production-ready for GitHub Actions verification. See ADR-006 for technical details. |
