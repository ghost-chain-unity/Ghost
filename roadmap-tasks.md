...
| **DEFER-1.1.3-1-nodeRPC** | TASK-1.1.3 (JSON-RPC) | Node-side Frontier RPC backend (fc-rpc, fc-db, fc-mapping-sync, fc-consensus) | Replit disk quota limits prevent git fetches from Moonbeam Frontier fork | **GitHub Actions** | ✅ RE-ENABLED | Node-side dependencies re-enabled on November 24, 2025. All 8 Frontier RPC crates (fc-rpc, fc-rpc-core, fc-db, fc-mapping-sync, fc-storage, fc-consensus, fp-rpc, fp-consensus) now active in node/Cargo.toml (lines 67-74). Full eth_* RPC backend ready for GitHub Actions compilation. |
