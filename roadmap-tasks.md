...
| **DEFER-1.1.3-1** | TASK-1.1.3 (JSON-RPC) | eth_* RPC methods (Ethereum compatibility) | Runtime pallets integrated but node-side infrastructure incomplete | 🚧 **IN PROGRESS** (November 24, 2025) | ⚠️ Incomplete | **PARTIALLY COMPLETE:** (1) ✅ Runtime: Frontier stable2412 pallets at indices 10-12, fp_rpc::EthereumRuntimeRPCApi + ConvertTransactionRuntimeApi implemented, (2) ✅ Node: fc-consensus/fp-consensus dependencies added, spawn_frontier_tasks function created, Frontier backend initialized, (3) ✅ RPC: Actual Eth/Net/Web3/EthFilter/EthPubSub/TxPool APIs wired in frontier.rs (NOT stub), (4) ⚠️ UNTESTED: Code not yet compiled due to disk quota. **Next:** Verify compilation, test eth_* endpoints. See ADR-006 for status. |
