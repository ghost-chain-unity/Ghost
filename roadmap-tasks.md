...
| **DEFER-1.1.3-1-nodeRPC** | TASK-1.1.3 (JSON-RPC) | Node-side Frontier RPC backend (fc-rpc, fc-db, fc-mapping-sync, fc-consensus) | ✅ **RESOLVED** (November 24, 2025) - Using moonbeam-polkadot-stable2506 branch | ✅ Complete | **STATUS RESOLVED:** (1) ✅ Runtime code complete (Frontier pallets in runtime, 212 lines), (2) ✅ RPC layer complete (frontier.rs 229 lines, all APIs wired), (3) ✅ Service.rs spawn_frontier_tasks fully implemented, (4) ✅ FIXED: Found Moonbeam Frontier branch moonbeam-polkadot-stable2506 with compatible RocksDB version (rocksdb 0.24 compatible). Frontier dependencies now uncommented in node/Cargo.toml with correct branch. **Status:** Node-side Frontier RPC backend ready for cargo update verification. |
