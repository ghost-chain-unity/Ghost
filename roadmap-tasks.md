...
| **DEFER-1.1.3-1-nodeRPC** | TASK-1.1.3 (JSON-RPC) | Node-side Frontier RPC backend (fc-rpc, fc-db, fc-mapping-sync, fc-consensus) | Substrate stable2503 branch incompatibility: sc-network/sc-network-types version mismatch (kad module missing) | **Phase 2 or Substrate Upgrade** | ⏳ PENDING | Node-side dependencies code-complete and documented in node/Cargo.toml (lines 66-74, commented out). Blocker: Polkadot SDK stable2503 branch uses sc-network 0.49+ which requires sc-network-types with kad module not in stable2503. **Action:** (1) Upgrade Polkadot SDK to newer stable branch (e.g., stable2504+), OR (2) Pin to compatible older versions. Runtime + RPC layer complete and functional; only node-side RPC orchestration blocked. All code ready, just awaiting version resolution. |
